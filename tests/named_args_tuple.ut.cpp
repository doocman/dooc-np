...
TEST(NamedArg, TupleCatView) // NOLINT
{
  ...
  static_assert(std::is_same_v<named_tuple_element_t<"a1", std::remove_reference_t<decltype(concat_tuple)>>, int>);
  static_assert(std::is_same_v<named_tuple_element_t<"a2", std::remove_reference_t<decltype(concat_tuple)>>, int>);
...
TEST(NamedArg, CatViewOfTransform) // NOLINT
{
  ...
  auto v2_tuple = transform([] (auto const& v, auto const&) { return v + 1; }, v2_tuple_raw);
  auto v3_tuple_raw = named_tuple(named_arg<"a3">(3), named_arg<"not used">(677));
